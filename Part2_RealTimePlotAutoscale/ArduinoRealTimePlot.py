...
class AnimationPlot:
    ...
    def animate(self,i, dataList, ser):
            ...
            if self.enableAutoscale == True:
                ax.autoscale()
            if self.enableAutoscale == False:
                ax.set_ylim([0, 1200])
                

    def getRadioSelect(self,selection):
            
            if selection == "Autoscale":
                self.enableAutoscale = True
            else:
                self.enableAutoscale = False
...
rax = plt.axes([0.0, 0.0, 0.15, 0.30],facecolor=axcolor)
radioAutoScale = RadioButtons(rax, ['Fixed','Autoscale'],activecolor='b')
radioAutoScale.on_clicked(realTimePlot.getRadioSelect)
...
ani = animation.FuncAnimation(fig, realTimePlot.animate, frames=100, fargs=(dataList, ser), interval=30) 
